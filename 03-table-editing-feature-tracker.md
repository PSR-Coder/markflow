...
| D5 | Selective history-to-current copy | Shipped, caveat | M3 | Side-by-side comparison supports left-side historical row selection, `Copy selected →`, and one-way per-row `→` copy arrows for changed historical rows. Copies use one CodeMirror transaction, recompute the diff, report the replacement/insertion count, and remain undoable. Structural Markdown selections are intentionally line-oriented for this first implementation. |
